...
const getState = ({ getStore, getActions, setStore }) => {
	return {
		...
		actions: {
			...
			getTotal: () => {
				const store = getStore();
				const res = store.carrito.reduce(
			...
			crearOrden: async (data) => {
				...
				if (validation === 'Missing Authorization Header')
					return 'Sesion expired';
				...
				try {
					...
					if (response.ok) {
						...
						setStore({
							...
							restore_authorization: responseJson.restore_URL,
						});
						return responseJson;
					} else {
						let responseJson = await response.json();
				...
				if (password.length > 7) {
					try {
						...
						if (response.ok) {
							let responseJson = await response.json();
							return {
								message: responseJson.message + '. Ahora puedes iniciar sesión',
								validation: 'ok',
							};
						} else {
							let responseJson = await response.json();
				...
				if (validation === 'Refresh successful' || 'Token still valid') {
					try {
						...
						if (firstResponse.ok) {
							...
							let secondResponse = await getActions().apiFetch(
								'logout',
								'POST'
							);
							if (secondResponse.ok) {
								...
								setStore({
									...
									loginDate: 0,
									user: '',
								}); //se resetea todo el store
								console.log('refresh_token revocado');
								return 'ok';
							} else {
								let refreshTokRevoked = await secondResponse.json();
								if (refreshTokRevoked != undefined)
									return refreshTokRevoked.message;
								else return 'Internal error';
							}
						} else {
							let accessTokRevoked = await firstResponse.json();
							if (accessTokRevoked != undefined)
								return accessTokRevoked.message;
							else return 'Internal error';
						}
					} catch (error) {
						console.error({ error });
        ...
        if (validation === "Refresh successful" || "Token still valid") {
          try {
			...
			tokenTimeValidation: async () => {
				...
				let timeSession =
					(process.env.JWT_ACCESS_TOKEN_EXPIRES_MINUTES - 1) * 60000;
